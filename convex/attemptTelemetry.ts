...
function suspiciousSessionFromRollup(rollup: {
  ...
  const duplicateRateBps =
    rollup.totalEvents > 0 ? Math.round((rollup.duplicateArtifacts * 10_000) / rollup.totalEvents) : 0;
  return duplicateRateBps >= 5000 || rollup.shadowBannedEvents > 0 || (
...
export const recordEventInternal = internalMutation({
  ...
  handler: async (ctx, args) => {
    ...
    const firstFullPass = nextPassCount > 0 && args.failCount === 0 && !existing.some((row) => row.firstFullPass);
    ...
    const usedValidationBeforeFinish =
      rollup?.usedValidationBeforeFinish === true || (isFinish && (rollup?.validateEvents ?? 0) > 0);
    ...
    const nextRollup = {
      ...
      totalPassDelta,
      duplicateArtifactRateBps: totalEvents > 0 ? Math.round((duplicateArtifacts * 10_000) / totalEvents) : 0,
  ...
  handler: async (ctx, args) => {
    ...
    return {
      ...
      events: events.map((event) => ({
        ...
        artifact: event.artifactId ? parseJsonField(artifactsById.get(event.artifactId)?.payloadJson) : null,
      })),
    };
  },
});
...
export const getFlaggedSessions = query({
  ...
  handler: async (ctx, args) => {
    ...
    return rows
      .filter((row) => row.suspiciousSession || row.shadowBannedEvents > 0 || row.duplicateArtifacts > 0)
...
export const getSessionsNeedingReview = query({
  ...
  handler: async (ctx, args) => {
    const rows = await ctx.db.query("sessionTelemetryRollups").collect();
    return rows
      .filter((row) => row.suspiciousSession || row.improvementRateBps === 0 || row.usedValidationBeforeFinish === false)
      .sort((a, b) => b.lastEventAt - a.lastEventAt)
      .slice(0, Math.max(1, Math.min(args.limit ?? 50, 200)));
  },
});
