...
import { SANDBOX_FLAG } from "../../src/lib/quickjs";
...
describe("/api/level-1/finish", () => {
  ...
  it("scores assigned submissions with real QuickJS evaluation and applies exploit or landmine modifiers", async () => {
    const { POST } = await import("../../src/app/api/level-1/finish/route");
    const req = new Request("http://localhost/api/level-1/finish", {
      method: "POST",
      headers: {
