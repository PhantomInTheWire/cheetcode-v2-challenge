...
vi.mock("../../src/lib/auth/admin-auth", () => ({
...
vi.mock("../../src/lib/abuse/kv", () => ({
  ...
  isKvConfigured: () => Boolean(hoisted.getKvClientMock()),
}));
...
describe("/api/admin/identity", () => {
  beforeEach(() => {
    ...
    process.env.CONVEX_MUTATION_SECRET = "test-secret";
  });
  ...
  it("accepts namespaced fingerprint identity keys for admin actions", async () => {
    hoisted.getKvClientMock.mockReturnValueOnce(null);
    const { POST } = await import("../../src/app/api/admin/identity/route");
    const res = await POST(
      new Request("http://localhost/api/admin/identity", {
        ...
        body: JSON.stringify({
          action: "shadow_ban",
          identityKeys: ["fp:id:fp-real-visitor-id"],
        }),
      }),
    );

    expect(res.status).toBe(200);
    expect(hoisted.localSetBanMock).toHaveBeenCalledWith("fp:id:fp-real-visitor-id");
  });
});
