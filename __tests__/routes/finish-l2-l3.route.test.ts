...
describe("finish l2/l3 routes", () => {
  ...
  it("/api/level-2/finish records results for authorized owner", async () => {
    const { POST } = await import("../../src/app/api/level-2/finish/route");
    ...
    const req = new Request("http://localhost/api/level-2/finish", {
      method: "POST",
  ...
  it("/api/level-2/finish ignores client-provided score bases", async () => {
    const { POST } = await import("../../src/app/api/level-2/finish/route");
    hoisted.queryMock.mockResolvedValueOnce({
      github: "tester",
      level: 2,
    ...
    const req = new Request("http://localhost/api/level-2/finish", {
      ...
      body: JSON.stringify({
        ...
        timeElapsed: 3000,
        runScoreSnapshot: { elo: 999999, solved: 999 },
      }),
    });

    const res = await POST(req);
    expect(res.status).toBe(200);
    const body = (await res.json()) as { elo: number; solved: number; rank: number };
    expect(body).toMatchObject({ elo: 123, solved: 1, rank: 1 });

    const actionCall = hoisted.actionMock.mock.calls[0]?.[1] as Record<string, unknown> | undefined;
    expect(actionCall).toBeTruthy();
    expect(actionCall).not.toHaveProperty("displayEloBase");
  });

  it("/api/level-2/finish rejects submissions after the expiry grace window", async () => {
  ...
  it("/api/level-2/finish rejects non-level-2 sessions", async () => {
    const { POST } = await import("../../src/app/api/level-2/finish/route");
    hoisted.queryMock.mockResolvedValueOnce({
      github: "tester",
      level: 1,
  ...
  it("/api/level-3/finish validates and records all solved checks for full L3 scoring input", async () => {
    const { POST } = await import("../../src/app/api/level-3/finish/route");
    const req = new Request("http://localhost/api/level-3/finish", {
      method: "POST",
      headers: { "content-type": "application/json" },
      body: JSON.stringify({
        sessionId: "s3",
  ...
  it("/api/level-3/finish short-circuits when shadow banned", async () => {
    const { POST } = await import("../../src/app/api/level-3/finish/route");
    const req = new Request("http://localhost/api/level-3/finish", {
      method: "POST",
      headers: {
  ...
  it("/api/level-3/finish extends expiry after stale-session failures", async () => {
    hoisted.validateL3Mock.mockResolvedValueOnce({
      compiled: true,
      staleSession: true,
      error: "session went stale",
      results: [],
    });
    ...
    expect(body).toEqual({ error: "session went stale", expiresAt: 130_000 });
    expect(hoisted.actionMock).toHaveBeenCalledTimes(2);
    const extensionCall = hoisted.actionMock.mock.calls[1]?.[1] as { extendMs: number } | undefined;
    expect(extensionCall?.extendMs).toBeTypeOf("number");
  });

  it("/api/level-3/finish rejects concurrent inflight submissions for the same identity", async () => {
    ...
    const req1 = new Request("http://localhost/api/level-3/finish", {
      method: "POST",
