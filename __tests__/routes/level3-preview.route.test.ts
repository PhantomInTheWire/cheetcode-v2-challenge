...
describe("/api/level-3/preview", () => {
  ...
  it("returns preview payload for authenticated request", async () => {
    const { GET } = await import("../../src/app/api/level-3/preview/route");
    const res = await GET(new Request("http://localhost/api/level-3/preview"));
