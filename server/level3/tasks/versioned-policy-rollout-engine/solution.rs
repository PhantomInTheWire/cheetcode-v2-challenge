...
fn find_binding<'a>(state: &'a mut State, subject_id: i32) -> Option<&'a mut Binding> {
    state.bindings.iter_mut().find(|b| b.subject_id == subject_id)
}

fn best_snapshot(state: &State, subject_id: i32, resource_id: i32, version: i32, ts: i64) -> Option<&Snapshot> {
    state.snapshots.iter().filter(|s| {
        s.subject_id == subject_id && s.resource_id == resource_id && s.version == version &&
        !s.retired && !s.disabled && ts >= s.not_before_ts && ts < s.expires_ts
...
#[no_mangle]
pub extern "C" fn policy_publish_snapshot(snapshot_id: i32, version: i32, subject_id: i32, resource_id: i32, allow_mask: i32, deny_mask: i32, priority: i32, not_before_ts: i64, expires_ts: i64) -> i32 {
    let mut state = STATE.lock().expect("lock");
    if state.snapshots.iter().any(|s| s.id == snapshot_id) {
        state.last_error = POLICY_ERR_DUPLICATE;
        return 0;
    }
    state.snapshots.push(Snapshot { id: snapshot_id, version, subject_id, resource_id, allow_mask, deny_mask, priority, retired: false, disabled: false, not_before_ts, expires_ts });
    state.last_error = POLICY_OK;
    1
}

#[no_mangle]
pub extern "C" fn policy_set_subject_binding(subject_id: i32, active_version: i32, fallback_version: i32) -> i32 {
    let mut state = STATE.lock().expect("lock");
    if let Some(binding) = find_binding(&mut state, subject_id) {
        ...
        state.bindings.push(Binding { subject_id, active_version, fallback_version, staged_version: 0 });
    }
    state.last_error = POLICY_OK;
    1
}
...
pub extern "C" fn policy_check(subject_id: i32, resource_id: i32, perm_bit: i32, ts: i64) -> i32 {
    ...
    let Some(binding) = state.bindings.iter().find(|b| b.subject_id == subject_id) else { return 0; };
    let snapshot = best_snapshot(&state, subject_id, resource_id, binding.active_version, ts)
        .or_else(|| best_snapshot(&state, subject_id, resource_id, binding.fallback_version, ts));
    let Some(snapshot) = snapshot else { return 0; };
    if snapshot.deny_mask & perm_bit != 0 { return 0; }
    if snapshot.allow_mask & perm_bit != 0 { 1 } else { 0 }
}

#[no_mangle]
pub extern "C" fn policy_explain_get(subject_id: i32, resource_id: i32, perm_bit: i32, ts: i64, out_view: *mut PolicyExplainView) -> i32 {
    if out_view.is_null() {
    ...
    let snapshot = if active.is_some() { active } else {
        let fb = best_snapshot(&state, subject_id, resource_id, binding.fallback_version, ts);
        if fb.is_some() { decided_version = binding.fallback_version; fallback_used = 1; }
        fb
    };
    let stale_snapshot = state.snapshots.iter().any(|s| s.subject_id == subject_id && s.resource_id == resource_id && s.version == binding.active_version && !s.retired && !s.disabled && (ts < s.not_before_ts || ts >= s.expires_ts)) as i32;
    let disabled_snapshot = state.snapshots.iter().any(|s| s.subject_id == subject_id && s.resource_id == resource_id && s.version == binding.active_version && (s.retired || s.disabled)) as i32;
    let mut view = PolicyExplainView { exists: 1, matched_snapshot_id: 0, decided_version, allow_mask: 0, deny_mask: 0, fallback_used, stale_snapshot, disabled_snapshot, usable: 0 };
    if let Some(snapshot) = snapshot {
        view.matched_snapshot_id = snapshot.id;
        view.allow_mask = snapshot.allow_mask;
        view.deny_mask = snapshot.deny_mask;
        view.usable = ((snapshot.allow_mask & perm_bit) != 0 && (snapshot.deny_mask & perm_bit) == 0) as i32;
    }
