...
fn subject_known(state: &State, subject_id: i32) -> bool {
    state.bindings.iter().any(|b| b.subject_id == subject_id)
        || state.snapshots.iter().any(|s| s.subject_id == subject_id)
}
...
pub extern "C" fn policy_explain_get(
    ...
    let mut state = STATE.lock().expect("lock");
    if !subject_known(&state, subject_id) {
        state.last_error = POLICY_ERR_UNKNOWN_SUBJECT;
        return 0;
    }
    let Some(binding) = state.bindings.iter().find(|b| b.subject_id == subject_id) else {
        unsafe {
            *out_view = PolicyExplainView {
                exists: 1,
