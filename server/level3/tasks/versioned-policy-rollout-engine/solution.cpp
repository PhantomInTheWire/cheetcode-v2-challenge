...
static Binding *binding_for(int subject_id, int create) {
  ...
  for (int i = 0; i < POLICY_MAX_BINDINGS; i++)
    if (!bindings[i].used) {
      std::memset(&bindings[i], 0, sizeof(Binding));
      bindings[i].used = 1;
...
static void active_flags(int subject_id, int resource_id, int version,
...
extern "C" __attribute__((visibility("default"))) void policy_reset(void) {
  std::memset(snapshots, 0, sizeof(snapshots));
...
policy_publish_snapshot(int snapshot_id, int version, int subject_id,
  ...
  for (int i = 0; i < POLICY_MAX_SNAPSHOTS; i++)
    if (!snapshots[i].used) {
      snapshots[i] = {1,           snapshot_id, version,       subject_id,
                      resource_id, allow_mask,  deny_mask,     priority,
                      0,           0,           not_before_ts, expires_ts};
      last_error = POLICY_OK;
      return 1;
    }
...
policy_explain_get(int subject_id, int resource_id, int perm_bit, int64_t ts,
  ...
  if (!b)
    return 0;
  out_view->exists = 1;
  active_flags(subject_id, resource_id, b->active_version, ts, &stale,
               &disabled);
  s = best_snapshot(subject_id, resource_id, b->active_version, ts);
  if (s)
...
extern "C" __attribute__((visibility("default"))) int policy_last_error(void) {
  return last_error;
}
