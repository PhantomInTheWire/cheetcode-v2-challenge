# Dependency Attestation Admission Gate
...
- Waivers are scoped to one `(service_id, environment_id)` pair.
- A waiver is active only while `ts < valid_until_ts`.
- An active waiver may bypass missing or stale healthy attestations.
- A waiver never bypasses a direct service block, a transitive dependency block, or conflicting evidence.
...
- `gate_audit_get(service_id, environment_id, ts, out_view)` returns `1` for known services and `0` otherwise.
- The audit payload must expose whether rollout is enabled, whether the service is attested, whether a waiver is active, and whether denial came from direct block, transitive block, stale evidence, or conflicting evidence.
- Known services must remain audit-visible even when rollout is disabled or admission is denied.
