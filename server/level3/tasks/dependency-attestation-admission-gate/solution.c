...
typedef struct {
  int used, id, blocked, dep_count, deps[8];
  int rollout[8];
} Service;
typedef struct {
...
static int env_slot(int environment_id) { return environment_id & 7; }
...
static void attestation_flags(int service_id, int env, int64_t ts, int *healthy,
...
static int admissible_inner(int service_id, int env, int64_t ts) {
  ...
  if (!s || !s->rollout[env_slot(env)] || s->blocked ||
    ...
    return 0;
  attestation_flags(service_id, env, ts, &healthy, &unhealthy, &stale);
...
__attribute__((visibility("default"))) void gate_reset(void) {
  ...
  memset(waivers, 0, sizeof(waivers));
  last_error = 0;
}
...
gate_report_attestation(int service_id, int environment_id, int status,
  ...
  for (int i = 0; i < G_MAX_ATTEST; i++)
    if (!attest[i].used) {
      attest[i].used = 1;
      attest[i].service_id = service_id;
      attest[i].environment_id = environment_id;
      attest[i].status = status;
      attest[i].observed_ts = observed_ts;
      attest[i].valid_until_ts = valid_until_ts;
      return 1;
    }
  return 0;
}
...
gate_add_waiver(int service_id, int environment_id, int64_t valid_until_ts) {
  ...
  for (int i = 0; i < G_MAX_WAIVERS; i++)
    if (!waivers[i].used) {
      waivers[i].used = 1;
      waivers[i].service_id = service_id;
      waivers[i].environment_id = environment_id;
      waivers[i].valid_until_ts = valid_until_ts;
      return 1;
    }
  return 0;
}
...
gate_audit_get(int service_id, int environment_id, int64_t ts,
  ...
  attestation_flags(service_id, environment_id, ts, &healthy, &unhealthy,
                    &stale);
