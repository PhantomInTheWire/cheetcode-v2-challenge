...
pub extern "C" fn cpu_assemble(
    ...
    if src.is_null() || out_words.is_null() || src_len <= 0 || max_words <= 0 {
        return -1;
    }
    let src_bytes = unsafe { slice::from_raw_parts(src as *const u8, src_len as usize) };
    let src_text = match str::from_utf8(src_bytes) {
        Ok(s) => s,
        Err(_) => return -2,
    };
    ...
    for line in src_text.lines() {
        ...
        match op {
            ...
            0x12 | 0x13 | 0x08 => {
                let rd = match parts.get(1).and_then(|t| parse_reg(t)) {
                    Some(v) => v,
                    None => return -6,
                };
            ...
            0x09 | 0x0A => {
                ...
                let imm = match parts.get(2).and_then(|t| resolve_value(t, &labels)) {
                    Some(v) => v,
                    None => return -8,
                };
            ...
            0x0C | 0x0D | 0x0E | 0x0F => {
                let addr = match parts.get(1).and_then(|t| resolve_value(t, &labels)) {
                    Some(v) => v,
                    None => return -9,
                };
                if !(0..=0x07FF).contains(&addr) {
                    return -16;
                }
            ...
            _ => {
                ...
                let rs = match parts.get(2).and_then(|t| parse_reg(t)) {
                    Some(v) => v,
                    None => return -14,
                };
