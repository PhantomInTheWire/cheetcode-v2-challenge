...
static int intern(const char *s) {
  for (int i = 0; i < E_MAX; i++)
    if (strs[i].used && std::strcmp(strs[i].v, s) == 0)
      return strs[i].id;
  for (int i = 0; i < E_MAX; i++)
    if (!strs[i].used) {
      strs[i].used = 1;
      strs[i].id = next_id++;
      std::snprintf(strs[i].v, sizeof(strs[i].v), "%s", s);
      return strs[i].id;
    }
  return 0;
}
static int eval_string_id_depth(int expr_id, int depth) {
  ...
  if (expr->kind == K_EMAIL) {
    ...
    if (!src)
      return 0;
    std::snprintf(buf, sizeof(buf), "%s", src->v);
    at = std::strchr(buf, '@');
    if (at)
      *at = '\0';
    return intern(buf);
  ...
  if (expr->kind == K_REPL) {
    ...
    std::snprintf(buf, sizeof(buf), "%s", src->v);
    pos = std::strstr(buf, pat->v);
    if (pos) {
      char out[128];
      *pos = '\0';
      std::snprintf(out, sizeof(out), "%s%s%s", buf, rep->v,
                    pos + std::strlen(pat->v));
      return intern(out);
    }
    return intern(buf);
  }
...
static int eval_match_value(int expr_id, int matcher_sid) {
  ...
  last_error = 0;
  return (std::strstr(val->v, pat->v) != nullptr) ^ (expr->c ? 1 : 0);
}
extern "C" __attribute__((visibility("default"))) void expr_reset(void) {
  std::memset(vars, 0, sizeof(vars));
  std::memset(strs, 0, sizeof(strs));
  std::memset(exprs, 0, sizeof(exprs));
  last_error = 0;
  next_id = 1000;
}
...
expr_register_string(int id, const char *value) {
  ...
  for (int i = 0; i < E_MAX; i++)
    if (!strs[i].used) {
      strs[i].used = 1;
      strs[i].id = id;
      std::snprintf(strs[i].v, sizeof(strs[i].v), "%s", value ? value : "");
      last_error = 0;
      return 1;
    }
  return 0;
...
extern "C" __attribute__((visibility("default"))) int
expr_register_var(int id, int ns, int sid) {
  if (varf(id)) {
    last_error = E_DUP;
    return 0;
  }
...
extern "C" __attribute__((visibility("default"))) int
expr_compile_literal(int id, int sid) {
  return add_expr(id, K_LIT, sid, 0, 0);
}
extern "C" __attribute__((visibility("default"))) int
expr_compile_var(int id, int vid) {
  return add_expr(id, K_VAR, vid, 0, 0);
}
extern "C" __attribute__((visibility("default"))) int
expr_compile_email_local(int id, int child) {
...
extern "C" __attribute__((visibility("default"))) int
expr_evaluate_string(int expr_id, int *out) {
  int sid;
  Expr *expr = exprf(expr_id);
  if (!out) {
    last_error = E_OUT;
...
extern "C" __attribute__((visibility("default"))) int expr_last_error(void) {
  return last_error;
}
