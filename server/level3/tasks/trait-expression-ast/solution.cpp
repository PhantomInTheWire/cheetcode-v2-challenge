...
static int namespace_error_depth(int expr_id, int depth);
...
static int namespace_error_depth(int expr_id, int depth) {
  ...
  if (expr->kind == K_VAR) {
    Var *var = varf(expr->a);
    return (!var || var->ns < 1 || var->ns > 3) ? 1 : 0;
  }
  if (expr->kind == K_EMAIL)
    return namespace_error_depth(expr->a, depth + 1);
  if (expr->kind == K_REPL || expr->kind == K_MATCH)
    return namespace_error_depth(expr->a, depth + 1);
  return 0;
}
...
expr_audit_get(int expr_id, int matcher_sid, ExprAuditView *out) {
  ...
  out->namespace_error = namespace_error_depth(expr_id, 0);
