...
typedef struct ExprAuditView {
  int exists, kind, string_evaluable, match_evaluable, constant_expr, namespace_error, matched,
    output_string_id;
} ExprAuditView;
...
int main(void) {
  ...
  seed();
  setc(&c[0], expr_evaluate_string(20, &sid) == 1 && sid == 1, "variable node resolves the registered trait string");
  ...
  seed();
  ok = expr_evaluate_string(27, &sid) == 1 && expr_audit_get(27, 0, &view) == 1 &&
    view.string_evaluable == 1 && view.output_string_id == sid;
  setc(&c[2], ok, "nested transforms compose and audit reports the resulting string id");
  ...
  seed();
  ok = expr_evaluate_string(24, &sid) == 0 && expr_audit_get(24, 0, &view) == 1 &&
    view.namespace_error == 1;
  setc(&c[4], ok, "invalid namespaces fail evaluation and remain audit-visible");

  seed();
  ok = expr_audit_get(25, 0, &view) == 1 && view.constant_expr == 1 && view.string_evaluable == 1;
  setc(&c[5], ok, "literal expression remains distinguishable from variable-driven expressions");

  seed();
  ok = expr_evaluate_match(26, 0) == 1 && expr_audit_get(26, 0, &view) == 1 && view.matched == 1;
  setc(&c[6], ok, "negated matcher flips the underlying non-match result");

  seed();
  ok = expr_evaluate_string(23, &sid) == 0 && expr_last_error() != 0;
  setc(&c[7], ok, "boolean matcher nodes are rejected from string-evaluation call sites");
  ...
  seed();
  ok = expr_compile_var(20, 10) == 0 && expr_last_error() != 0 && expr_audit_get(999, 0, &view) == 0;
  setc(&c[10], ok, "duplicate expression ids and unknown audit targets fail cleanly");
