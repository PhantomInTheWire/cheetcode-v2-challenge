...
struct Session {
  int used, id, subject_id, resource_id, active_generation, staged_generation,
      grace_generation, revoked, rev_gens[8];
...
session_issue_credential(int credential_id, int session_id, int generation,
  ...
  for (int i = 0; i < S_MAX_CREDS; i++)
    if (!creds[i].used) {
      creds[i] = {1,          credential_id, session_id,
                  generation, issued_ts,     expires_ts};
      return 1;
    }
  return 0;
}
...
extern "C" __attribute__((visibility("default"))) int
session_revoke(int session_id, int generation) {
  Session *s = session_by_id(session_id);
  if (!s) {
    last_error = S_ERR_UNKNOWN;
    return 0;
...
extern "C" __attribute__((visibility("default"))) int
session_count_active(int subject_id, int64_t ts) {
  int count = 0;
  for (int i = 0; i < S_MAX_SESSIONS; i++)
