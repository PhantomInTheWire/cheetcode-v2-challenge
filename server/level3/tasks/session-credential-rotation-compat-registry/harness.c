...
int session_create(int session_id, int subject_id, int resource_id,
                   int active_generation);
int session_issue_credential(int credential_id, int session_id, int generation,
                             int64_t issued_ts, int64_t expires_ts);
int session_stage_generation(int session_id, int generation,
...
int session_audit_get(int session_id, int generation, int64_t ts,
...
static void seed_world(void) {
  session_reset();
  session_create(9, 4, 70, 1);
  session_issue_credential(1, 9, 1, 0, 100);
  session_stage_generation(9, 2, 50);
  session_issue_credential(2, 9, 2, 10, 100);
}
int main(void) {
  ...
  int ok;
  seed_world();
  set_check(&checks[0], session_check(9, 1, 10) == 1,
            "active generation authorizes");
  session_activate_generation(9, 20);
  set_check(&checks[1],
            session_check(9, 1, 25) == 1 && session_check(9, 1, 60) == 0,
            "grace window supports lagging clients briefly");
  set_check(&checks[2], session_check(9, 2, 120) == 0,
            "expired credentials deny access");
  ok = session_audit_get(9, 1, 25, &view) == 1 && view.grace_active == 1 &&
       view.compatible == 1;
  set_check(&checks[3], ok, "audit exposes grace compatibility");
  seed_world();
  session_activate_generation(9, 20);
  set_check(&checks[4], session_check(9, 2, 25) == 1,
            "activation promotes staged generation");
  session_revoke(9, 1);
  set_check(&checks[5], session_check(9, 1, 25) == 0,
            "revocation overrides grace");
  seed_world();
  session_create(10, 4, 71, 1);
  session_issue_credential(3, 10, 1, 0, 100);
  set_check(&checks[6], session_count_active(4, 10) == 2,
            "independent sessions remain visible");
  set_check(&checks[7], session_count_active(4, 10) == 2,
            "summary path stable");
  for (int i = 0; i < 600; i++) {
    session_create(100 + i, 900 + i, 88, 1);
    session_issue_credential(1000 + i, 100 + i, 1, 0, 100);
  }
  set_check(&checks[8], session_check(9, 1, 10) == 1,
            "lookup stable with noisy session population");
  ok = session_audit_get(9, 2, 25, &view) == 1 && view.active_generation == 2;
  set_check(&checks[9], ok, "audit remains operator-friendly");
  set_check(&checks[10], session_count_active(4, 10) >= 1,
            "count path stays correct");
  seed_world();
  session_activate_generation(9, 20);
  session_revoke(9, -1);
  set_check(&checks[11],
            session_check(9, 2, 25) == 0 && session_check(9, 1, 25) == 0,
            "session-wide revoke kills active and grace generations");
  seed_world();
  session_activate_generation(9, 20);
  session_revoke(9, 1);
  set_check(&checks[12], session_check(9, 2, 25) == 1,
            "revoking grace generation does not break new active generation");
  seed_world();
  session_activate_generation(9, 20);
  ok = session_audit_get(9, 1, 25, &view) == 1 && view.grace_active == 1 &&
       view.usable == 1;
  set_check(
      &checks[13], ok,
      "audit distinguishes usable grace generation from active generation");
  for (int i = 14; i < 25; i++)
