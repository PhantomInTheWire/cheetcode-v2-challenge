...
int auth_create_local_grant(int grant_id, int subject_id, int resource_id,
                            int perms_mask, int64_t not_before_ts,
                            int64_t expires_ts, int delegatable);
int auth_import_bundle_grant(int grant_id, int subject_id, int resource_id,
                             int perms_mask, int64_t not_before_ts,
                             int64_t expires_ts, int delegatable,
                             int requires_key);
int auth_attach_bundle_key(int grant_id);
int auth_delegate(int parent_grant_id, int child_grant_id, int subject_id,
...
int auth_check(int subject_id, int resource_id, int perm_bit, int64_t ts,
               int resolve_mode);
...
static void ref_set_error(int code) { ref_last_error = code; }
...
static int ref_find_index(int grant_id) {
  for (int i = 0; i < REF_MAX_GRANTS; i++) {
    if (ref_grants[i].used && ref_grants[i].id == grant_id)
      return i;
  }
  return -1;
}
...
static int ref_select_source(int subject_id, int resolve_mode) {
  ...
  for (int i = 0; i < REF_MAX_GRANTS; i++) {
    if (ref_grants[i].used && ref_grants[i].subject_id == subject_id &&
...
static void ref_write_grant(int index, int grant_id, int parent_index,
...
static int ref_create_local_grant(int grant_id, int subject_id, int resource_id,
                                  int perms_mask, int64_t not_before_ts,
                                  int64_t expires_ts, int delegatable) {
  int index;
  ...
  ref_write_grant(index, grant_id, -1, subject_id, resource_id,
...
static int ref_import_bundle_grant(int grant_id, int subject_id,
                                   int resource_id, int perms_mask,
...
static int ref_delegate(int parent_grant_id, int child_grant_id, int subject_id,
                        int resource_id, int perms_mask, int64_t not_before_ts,
                        int64_t expires_ts, int delegatable, int requires_key) {
  int parent_index;
  int child_index;
...
static int ref_check(int subject_id, int resource_id, int perm_bit, int64_t ts,
                     int resolve_mode) {
  ...
  for (int i = 0; i < REF_MAX_GRANTS; i++) {
    if (ref_grants[i].used && ref_grants[i].subject_id == subject_id &&
        ...
        ref_grants[i].source == source && requested != 0 &&
...
static int ref_count_usable(int subject_id, int64_t ts, int resolve_mode) {
  ...
  for (int i = 0; i < REF_MAX_GRANTS; i++) {
    if (ref_grants[i].used && ref_grants[i].subject_id == subject_id &&
        ref_grants[i].source == source && ref_effective_mask(i, ts) != 0) {
      count++;
    }
  }
  ref_set_error(AUTH_OK);
...
static int view_equal(const AuthAuditView *a, const AuthAuditView *b) {
  return a->exists == b->exists && a->source == b->source &&
         a->stored_mask == b->stored_mask &&
         a->effective_mask == b->effective_mask && a->revoked == b->revoked &&
         a->requires_key == b->requires_key &&
         a->key_attached == b->key_attached &&
         a->not_yet_valid == b->not_yet_valid && a->expired == b->expired &&
...
static void track_hot_id(int id) {
  if (hot_id_count < HOT_ID_CAP)
    hot_ids[hot_id_count++] = id;
}

static void track_leaf_id(int id) {
  if (leaf_id_count < LEAF_ID_CAP)
    leaf_ids[leaf_id_count++] = id;
}

static double measure_read_workload(int (*workload)(int), int iterations,
                                    int *checksum) {
  clock_t start_clock;
  clock_t end_clock;
  workload(512);
...
static void set_ratio_result(Check *check, const char *label, double baseline,
                             double inflated, double max_ratio,
                             int baseline_sum, int inflated_sum) {
  ...
  snprintf(check->msg, sizeof(check->msg),
           "%s baseline=%.4fs inflated=%.4fs ratio=%.2fx<=%.2fx sum=%d/%d",
           label, baseline, inflated, ratio, max_ratio, baseline_sum,
           inflated_sum);
  check->ok = ok ? 1 : 0;
}

static void set_budget_result(Check *check, const char *label, double elapsed,
                              double budget, int checksum) {
  snprintf(check->msg, sizeof(check->msg), "%s elapsed=%.4fs<=%.4fs sum=%d",
           label, elapsed, budget, checksum);
...
static void seed_lookup_world(int cold_noise_grants) {
  ...
  for (int subject = 0; subject < 16; subject++) {
    for (int resource = 0; resource < 4; resource++) {
      ...
      auth_create_local_grant(local_id, subject, resource,
                              AUTH_PERM_READ | AUTH_PERM_WRITE, 0, 200, 0);
      auth_import_bundle_grant(bundle_id, subject, resource, AUTH_PERM_READ, 0,
                               200, 0, (subject + resource) % 3 == 0);
      if ((subject + resource) % 3 != 0)
        auth_attach_bundle_key(bundle_id);
  ...
  for (int i = 0; i < cold_noise_grants; i++) {
    ...
    if (i & 1) {
      auth_import_bundle_grant(next_id++, subject, resource, AUTH_PERM_READ, 0,
                               200, 0, 0);
    } else {
      auth_create_local_grant(next_id++, subject, resource, AUTH_PERM_READ, 0,
                              200, 0);
    }
  }
}

static void seed_auth_world(int world_kind, int same_subject_noise_per_subject,
                            int cold_noise_grants) {
  int next_id = 1000;
  reset_tracks();
  auth_reset();
  for (int subject = 0; subject < 16; subject++) {
    for (int resource = 0; resource < 4; resource++) {
      ...
      int use_bundle = world_kind == WORLD_BUNDLE_HEAVY ||
                       world_kind == WORLD_AUTO_ALL_BUNDLE ||
                       (world_kind == WORLD_MIXED && (subject % 2 == 0));
      auth_create_local_grant(local_id, subject, resource,
                              AUTH_PERM_READ | AUTH_PERM_WRITE, 0, 200, 1);
      if (use_bundle) {
        int bundle_id = next_id++;
        auth_import_bundle_grant(bundle_id, subject, resource, AUTH_PERM_READ,
                                 0, 200, 1, 0);
    ...
    for (int extra = 0; extra < same_subject_noise_per_subject; extra++) {
      ...
      int use_bundle = world_kind == WORLD_BUNDLE_HEAVY ||
                       ...
                       (world_kind == WORLD_MIXED && (subject % 2 == 0));
      if (use_bundle) {
        auth_import_bundle_grant(next_id++, subject, resource, AUTH_PERM_READ,
                                 0, 200, 0, 0);
      } else {
        auth_create_local_grant(next_id++, subject, resource, AUTH_PERM_READ, 0,
                                200, 0);
...
static void seed_count_world(int mixed_mode, int cold_noise_grants) {
  ...
  for (int subject = 0; subject < 12; subject++) {
    for (int resource = 0; resource < 16; resource++) {
      int local_id = next_id++;
      auth_create_local_grant(local_id, subject, resource, AUTH_PERM_READ, 0,
                              200, 0);
      track_hot_id(local_id);
...
static void seed_deep_chain_world(int cold_noise_grants) {
  ...
  for (int subject = 0; subject < 16; subject++) {
    for (int resource = 0; resource < 2; resource++) {
      int parent = next_id++;
      auth_create_local_grant(parent, subject, resource, AUTH_PERM_READ, 0, 200,
                              1);
      for (int depth = 1; depth < 8; depth++) {
        int child = next_id++;
        auth_delegate(parent, child, subject, resource, AUTH_PERM_READ, 0, 200,
                      depth < 7, 0);
        parent = child;
      }
      track_hot_id(parent);
...
static void seed_large_world(int cold_noise_grants) {
  ...
  for (int subject = 0; subject < 32; subject++) {
    for (int resource = 0; resource < 8; resource++) {
      ...
      int extra_a = next_id++;
      int extra_b = next_id++;
      auth_create_local_grant(root_local, subject, resource,
                              AUTH_PERM_READ | AUTH_PERM_WRITE, 0, 400, 1);
      auth_delegate(root_local, child_local, subject, resource, AUTH_PERM_READ,
                    0, 400, 0, 0);
      if (subject % 2 == 0) {
        auth_import_bundle_grant(extra_a, subject, resource, AUTH_PERM_READ, 0,
                                 400, 1, 0);
        auth_delegate(extra_a, extra_b, subject, resource, AUTH_PERM_READ, 0,
                      400, 0, 0);
      } else {
        auth_create_local_grant(extra_a, subject, resource, AUTH_PERM_READ, 0,
                                400, 0);
        auth_create_local_grant(extra_b, subject, resource, AUTH_PERM_READ, 0,
                                400, 0);
      }
...
static int workload_deep_chain_hot_read(int iterations) {
  ...
  for (int i = 0; i < iterations; i++) {
    ...
    sum +=
        auth_check(subject, resource, AUTH_PERM_READ, 50, AUTH_MODE_LOCAL_ONLY);
  }
  return sum;
}
...
int main(void) {
  ...
  auth_reset();
  if (auth_create_local_grant(1, 1, 100, AUTH_PERM_WRITE, 0, 40, 0) == 1 &&
    ...
    auth_reset();
    if (auth_create_local_grant(3, 2, 101, AUTH_PERM_READ, 0, 40, 0) == 1 &&
      ...
      check_set(&behavior_source_resolution, 1,
                "source precedence and mode isolation hold");
    } else {
      check_set(&behavior_source_resolution, 0,
                "AUTO fallback or mode isolation failed");
    }
  } else {
    check_set(&behavior_source_resolution, 0,
              "source resolution fixture failed");
  }

  auth_reset();
  if (auth_create_local_grant(10, 3, 102, AUTH_PERM_ADMIN, 10, 20, 0) == 1 &&
    ...
    auth_reset();
    if (auth_create_local_grant(11, 3, 102, AUTH_PERM_READ, 10, 20, 0) == 1 &&
        auth_check(3, 102, AUTH_PERM_READ, 10, AUTH_MODE_LOCAL_ONLY) == 1 &&
        auth_check(3, 102, AUTH_PERM_READ, 9, AUTH_MODE_LOCAL_ONLY) == 0) {
      check_set(&behavior_time_and_perm_contract, 1,
                "time boundaries and ADMIN contract hold");
    } else {
      check_set(&behavior_time_and_perm_contract, 0,
                "time boundary behavior failed");
    }
  } else {
  ...
  auth_reset();
  if (auth_create_local_grant(20, 4, 103, AUTH_PERM_READ, 0, 40, 1) == 1 &&
      auth_delegate(20, 21, 4, 103, AUTH_PERM_READ, 0, 40, 1, 0) == 1 &&
      auth_revoke(20) == 1 &&
      auth_check(4, 103, AUTH_PERM_READ, 10, AUTH_MODE_LOCAL_ONLY) == 0) {
    auth_reset();
    if (auth_import_bundle_grant(22, 5, 104, AUTH_PERM_READ, 0, 40, 1, 1) ==
            1 &&
        ...
        auth_delegate(22, 24, 5, 104, AUTH_PERM_READ | AUTH_PERM_WRITE, 0, 40,
                      0, 0) == 0 &&
        auth_last_error() == AUTH_ERR_CHILD_MASK_WIDENS) {
      check_set(&behavior_delegation_and_ancestors, 1,
                "delegation bounds and ancestor disablement hold");
    } else {
      check_set(&behavior_delegation_and_ancestors, 0,
                "delegation or ancestor rules failed");
    }
  } else {
    check_set(&behavior_delegation_and_ancestors, 0,
              "revoke propagation failed");
  }

  auth_reset();
  {
    AuthAuditView view;
    if (auth_create_local_grant(30, 6, 105, AUTH_PERM_READ, 0, 40, 0) == 1 &&
        auth_attach_bundle_key(30) == 0 &&
        auth_last_error() == AUTH_ERR_WRONG_SOURCE) {
      auth_reset();
      if (auth_import_bundle_grant(31, 6, 105, AUTH_PERM_READ, 0, 40, 0, 1) ==
              1 &&
          auth_audit_get(31, 10, &view) == 1 && view.exists == 1 &&
          view.requires_key == 1 && view.key_attached == 0 &&
          view.usable == 0 &&
        ...
        check_set(&behavior_audit_count_error_contract, 1,
                  "audit/count/error contract holds");
      } else {
        check_set(&behavior_audit_count_error_contract, 0,
                  "audit/count/error contract failed");
      }
    } else {
      check_set(&behavior_audit_count_error_contract, 0,
                "error surface fixture failed");
    }
  }
  ...
  {
    AuthAuditView view;
    if (auth_create_local_grant(40, 7, 106, AUTH_PERM_READ, 0, 40, 1) == 1 &&
        ...
        auth_audit_get(41, 10, &view) == 1 && view.disabled_by_ancestor == 1 &&
        view.revoked == 0 &&
        auth_count_usable(7, 10, AUTH_MODE_LOCAL_ONLY) == 0) {
      check_set(&update_revoke_read_consistency, 1,
                "revoke updates every read path");
    } else {
      check_set(&update_revoke_read_consistency, 0,
                "revoke left stale read-path state");
    }
  }
  ...
  {
    AuthAuditView view;
    if (auth_create_local_grant(50, 8, 107, AUTH_PERM_READ, 0, 40, 0) == 1 &&
        auth_import_bundle_grant(51, 8, 107, AUTH_PERM_READ, 0, 40, 0, 1) ==
            1 &&
        ...
        auth_audit_get(51, 10, &view) == 1 && view.key_attached == 1 &&
        view.usable == 1) {
      check_set(&update_key_attach_read_consistency, 1,
                "key attach updates auth state consistently");
    } else {
      check_set(&update_key_attach_read_consistency, 0,
                "key attach did not refresh read paths");
    }
  }

  auth_reset();
  if (auth_create_local_grant(60, 9, 108, AUTH_PERM_READ, 0, 40, 1) == 1 &&
    ...
    check_set(&update_independent_chain_isolation, 1,
              "independent chains stay isolated");
  } else {
    check_set(&update_independent_chain_isolation, 0,
              "revoke spilled into independent chain");
  }

  {
    ...
    for (int step = 0; step < 320 && ok; step++) {
      ...
      if (op == 0) {
        ...
        ok = ref_create_local_grant(id, subject, resource, mask, start, end,
                                    delegatable) ==
                 auth_create_local_grant(id, subject, resource, mask, start,
                                         end, delegatable) &&
        ...
        ok = ref_import_bundle_grant(id, subject, resource, mask, start, end,
                                     delegatable, requires_key) ==
                 auth_import_bundle_grant(id, subject, resource, mask, start,
                                          end, delegatable, requires_key) &&
             ref_last_error == auth_last_error();
      } else if (op == 2) {
        int id = next_id > 1000
        ...
        int delegatable = (int)(lcg_next(&rng) & 1u);
        int requires_key = (int)(lcg_next(&rng) & 1u);
        ok = ref_delegate(parent_id, child_id, subject, resource, mask, start,
                          end, delegatable, requires_key) ==
                 auth_delegate(parent_id, child_id, subject, resource, mask,
                               start, end, delegatable, requires_key) &&
             ref_last_error == auth_last_error();
      } else if (op == 4) {
        int id = next_id > 1000
                     ? 1000 + (int)(lcg_next(&rng) % (uint32_t)(next_id - 1000))
                     : 99;
        ok = ref_revoke(id) == auth_revoke(id) &&
             ref_last_error == auth_last_error();
      } else if (op == 5) {
        int subject = (int)(lcg_next(&rng) % 10u);
        int resource = (int)(lcg_next(&rng) % 6u);
        ...
        ok = ref_audit_get(id, ts, &ref_view) ==
                 auth_audit_get(id, ts, &actual_view) &&
             (!ref_audit_get(id, ts, &ref_view) ||
              view_equal(&ref_view, &actual_view)) &&
             ref_count_usable(subject, ts, mode) ==
                 auth_count_usable(subject, ts, mode) &&
             ref_last_error == auth_last_error();
      }
    }
    check_set(&update_small_trace_equivalence, ok,
              ok ? "small randomized trace matches reference"
                 : "small randomized trace diverged");
  }

  {
    ...
    seed_lookup_world(0);
    base_elapsed = measure_read_workload(workload_grant_id_lookup_ratio, 100000,
                                         &base_sum);
    seed_lookup_world(3072);
    inflated_elapsed = measure_read_workload(workload_grant_id_lookup_ratio,
                                             100000, &inflated_sum);
    set_ratio_result(&scale_grant_id_lookup_ratio, "id-lookup", base_elapsed,
                     inflated_elapsed, 2.0, base_sum, inflated_sum);
  }

  {
    ...
    seed_lookup_world(0);
    base_elapsed =
        measure_read_workload(workload_audit_lookup_ratio, 80000, &base_sum);
    seed_lookup_world(3072);
    inflated_elapsed = measure_read_workload(workload_audit_lookup_ratio, 80000,
                                             &inflated_sum);
    set_ratio_result(&scale_audit_lookup_ratio, "audit-lookup", base_elapsed,
                     inflated_elapsed, 2.0, base_sum, inflated_sum);
  }

  {
    ...
    double inflated_elapsed;
    seed_lookup_world(0);
    base_elapsed = measure_read_workload(workload_effective_mask_lookup_ratio,
                                         80000, &base_sum);
    seed_lookup_world(3072);
    inflated_elapsed = measure_read_workload(
        workload_effective_mask_lookup_ratio, 80000, &inflated_sum);
    set_ratio_result(&scale_effective_mask_lookup_ratio, "effective-mask",
                     base_elapsed, inflated_elapsed, 2.0, base_sum,
                     inflated_sum);
  }

  {
    ...
    base_elapsed =
        measure_read_workload(workload_auto_hot_auth, 120000, &base_sum);
    seed_auth_world(WORLD_AUTO_ALL_BUNDLE, 0, 3072);
    inflated_elapsed =
        measure_read_workload(workload_auto_hot_auth, 120000, &inflated_sum);
    set_ratio_result(&scale_auto_bundle_presence_other_subject_noise,
                     "auto-other-noise", base_elapsed, inflated_elapsed, 2.5,
                     base_sum, inflated_sum);
  }

  {
    ...
    inflated_elapsed =
        measure_read_workload(workload_auto_hot_auth, 120000, &inflated_sum);
    set_ratio_result(&scale_auto_bundle_presence_same_subject_noise,
                     "auto-same-subject-noise", base_elapsed, inflated_elapsed,
                     2.5, base_sum, inflated_sum);
  }

  {
    ...
    inflated_elapsed =
        measure_read_workload(workload_auto_hot_auth, 120000, &inflated_sum);
    set_ratio_result(&scale_hot_auth_lookup_ratio, "mixed-hot-auth",
                     base_elapsed, inflated_elapsed, 2.5, base_sum,
                     inflated_sum);
  }

  {
    ...
    inflated_elapsed =
        measure_read_workload(workload_auto_hot_auth, 120000, &inflated_sum);
    set_ratio_result(&scale_same_subject_irrelevant_resource_ratio,
                     "same-subject-resource-noise", base_elapsed,
                     inflated_elapsed, 2.5, base_sum, inflated_sum);
  }

  {
    ...
    inflated_elapsed =
        measure_read_workload(workload_auto_hot_auth, 120000, &inflated_sum);
    set_ratio_result(&scale_other_subject_noise_ratio, "other-subject-noise",
                     base_elapsed, inflated_elapsed, 2.5, base_sum,
                     inflated_sum);
  }

  {
    ...
    base_elapsed =
        measure_read_workload(workload_count_hot_subject, 30000, &base_sum);
    seed_count_world(0, 3072);
    inflated_elapsed =
        measure_read_workload(workload_count_hot_subject, 30000, &inflated_sum);
    set_ratio_result(&scale_count_usable_hot_subject_ratio, "count-hot-subject",
                     base_elapsed, inflated_elapsed, 2.5, base_sum,
                     inflated_sum);
  }

  {
    ...
    base_elapsed =
        measure_read_workload(workload_count_mode_mix, 30000, &base_sum);
    seed_count_world(1, 3072);
    inflated_elapsed =
        measure_read_workload(workload_count_mode_mix, 30000, &inflated_sum);
    set_ratio_result(&scale_count_usable_mode_mix_ratio, "count-mode-mix",
                     base_elapsed, inflated_elapsed, 2.5, base_sum,
                     inflated_sum);
  }

  {
    ...
    base_elapsed =
        measure_read_workload(workload_mixed_read_loop, 50000, &base_sum);
    seed_auth_world(WORLD_MIXED, 32, 3072);
    inflated_elapsed =
        measure_read_workload(workload_mixed_read_loop, 50000, &inflated_sum);
    set_ratio_result(&scale_mixed_read_loop_ratio, "mixed-read-loop",
                     base_elapsed, inflated_elapsed, 2.5, base_sum,
                     inflated_sum);
  }

  {
    ...
    base_elapsed =
        measure_read_workload(workload_deep_chain_hot_read, 60000, &base_sum);
    seed_deep_chain_world(3072);
    inflated_elapsed = measure_read_workload(workload_deep_chain_hot_read,
                                             60000, &inflated_sum);
    set_ratio_result(&scale_deep_chain_hot_read_ratio, "deep-chain-hot-read",
                     base_elapsed, inflated_elapsed, 2.5, base_sum,
                     inflated_sum);
  }

  {
    ...
    base_elapsed =
        measure_read_workload(workload_bundle_hot_auth, 120000, &base_sum);
    seed_auth_world(WORLD_BUNDLE_HEAVY, 16, 3072);
    inflated_elapsed =
        measure_read_workload(workload_bundle_hot_auth, 120000, &inflated_sum);
    set_ratio_result(&scale_bundle_heavy_hotset_ratio, "bundle-heavy-hotset",
                     base_elapsed, inflated_elapsed, 2.5, base_sum,
                     inflated_sum);
  }

  {
    ...
    base_elapsed =
        measure_read_workload(workload_local_hot_auth, 120000, &base_sum);
    seed_auth_world(WORLD_LOCAL_HEAVY, 16, 3072);
    inflated_elapsed =
        measure_read_workload(workload_local_hot_auth, 120000, &inflated_sum);
    set_ratio_result(&scale_local_heavy_hotset_ratio, "local-heavy-hotset",
                     base_elapsed, inflated_elapsed, 2.5, base_sum,
                     inflated_sum);
  }

  {
    ...
    inflated_elapsed =
        measure_read_workload(workload_auto_hot_auth, 120000, &inflated_sum);
    set_ratio_result(&scale_mixed_source_hotset_ratio, "mixed-source-hotset",
                     base_elapsed, inflated_elapsed, 2.5, base_sum,
                     inflated_sum);
  }

  {
    ...
    set_budget_result(&scale_large_read_budget, "large-read-budget", elapsed,
                      1.20, checksum);
  }

  {
    ...
    if (ok) {
      ...
      snprintf(scale_large_trace_equivalence_budget.msg,
               sizeof(scale_large_trace_equivalence_budget.msg),
               "trace=ok elapsed=%.4fs<=%.4fs sum=%d", elapsed, 1.40, checksum);
      scale_large_trace_equivalence_budget.ok = elapsed <= 1.40 ? 1 : 0;
    } else {
      check_set(&scale_large_trace_equivalence_budget, 0,
                "large trace diverged from reference");
    }
  }

  print_check("behavior_source_resolution", &behavior_source_resolution);
  print_check("behavior_time_and_perm_contract",
              &behavior_time_and_perm_contract);
  print_check("behavior_delegation_and_ancestors",
              &behavior_delegation_and_ancestors);
  print_check("behavior_audit_count_error_contract",
              &behavior_audit_count_error_contract);
  print_check("update_revoke_read_consistency",
              &update_revoke_read_consistency);
  print_check("update_key_attach_read_consistency",
              &update_key_attach_read_consistency);
  print_check("update_independent_chain_isolation",
              &update_independent_chain_isolation);
  print_check("update_small_trace_equivalence",
