...
const SectionHead = ({ title, description, action }) => (
  <div className="flex justify-between items-start mb-24">
    <div>
      ...
      {description && (
        <p className="body-large text-accent-black/60 max-w-[590px] mt-8">{description}</p>
      )}
    </div>
...
const SubsectionHead = ({ title, description }) => (
  <div className="mb-16">
    ...
    {description && <p className="body-small text-accent-black/50">{description}</p>}
  </div>
);
```
...
const Input = ({ label, error, ...props }) => (
  <div className="flex flex-col gap-6">
    {label && <label className="label-small text-accent-black">{label}</label>}
    <input
      className={`
        h-40
        px-12
      ...
      {...props}
    />
    {error && <span className="body-small text-crimson">{error}</span>}
  </div>
);
```
