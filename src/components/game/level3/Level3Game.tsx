...
import { L3_MARKDOWN_COMPONENTS } from "@/components/game/level3/level3MarkdownComponents";
...
export function Level3Game({
  ...
  async function runChecks() {
    ...
    try {
      const res = await clientFetch("/api/level-3/validate", {
        method: "POST",
  ...
  const finishGame = useCallback(async () => {
    ...
    setSubmitError(null);
    try {
      const finishRes = await clientFetch("/api/level-3/finish", {
        method: "POST",
        headers: { "content-type": "application/json" },
        body: JSON.stringify({
          sessionId,
