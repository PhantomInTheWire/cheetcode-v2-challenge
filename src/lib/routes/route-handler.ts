...
import { requireOwnedSession, type OwnedSession } from "../session/session-auth";
...
function hasSessionId(body: unknown): body is { sessionId: string } {
  return (
    !!body &&
    typeof body === "object" &&
...
export async function withAuthenticatedSession<TBody extends { sessionId: string }>(
  ...
  return withOwnedSessionRoute<TBody>(
    request,
    {
      expectedLevel: options.expectedLevel,
      expiryGraceMs: options.expiryGraceMs ?? FINISH_ROUTE_EXPIRY_GRACE_MS,
      validateBody: (body): body is TBody => hasSessionId(body) && options.validateBody(body),
      errorLabel: options.errorLabel ?? `Route handler error (Level ${options.expectedLevel})`,
      errorResponse:
        options.errorResponse ??
        NextResponse.json(
