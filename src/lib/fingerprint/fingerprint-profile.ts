...
let lastCollectedSummary: CompactFingerprintSummary | null = null;
...
export function getCachedFingerprintSummary(): CompactFingerprintSummary | null {
  return lastCollectedSummary;
}

export async function ensureClientFingerprintProfile(): Promise<ClientFingerprintProfile> {
  if (!fingerprintProfilePromise) {
    fingerprintProfilePromise = collectFingerprintProfile()
      .then((profile) => {
        lastCollectedSummary = buildCompactFingerprintSummary(profile);
        return profile;
      })
      .finally(() => {
        fingerprintProfilePromise = null;
      });
  }
