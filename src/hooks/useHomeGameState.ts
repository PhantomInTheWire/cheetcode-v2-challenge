...
export function useHomeGameState({
  ...
  const finishGame = useCallback(async () => {
    ...
    setSubmitError(null);
    try {
      const res = await clientFetch("/api/level-1/finish", {
        method: "POST",
      ...
      clearStoredSession();
      if (data.completedLevel === true) {
        updateRunScoreSnapshot(data.scoreSnapshot ?? null);
    ...
    async function restoreSession() {
      ...
      try {
        ...
        const res = await clientFetch("/api/session/restore", {
          method: "POST",
          headers: { "content-type": "application/json" },
          body: JSON.stringify({ sessionId: persisted?.sessionId, fingerprintHints }),
  ...
  useEffect(() => {
    ...
    async function loadLevel2Preview() {
      ...
      setLevel2PreviewError(null);
      try {
        const res = await clientFetch("/api/level-2/preview", { method: "GET" });
        const data = await res.json().catch(() => ({}));
        if (!res.ok) {
  ...
  useEffect(() => {
    ...
    async function loadLevel3Preview() {
      ...
      setLevel3PreviewError(null);
      try {
        const res = await clientFetch("/api/level-3/preview", { method: "GET" });
        const data = await res.json().catch(() => ({}));
        if (!res.ok) {
  ...
  const runLocalCheck = useCallback(
    async (problem: GameProblem) => {
      ...
      try {
        const res = await clientFetch("/api/level-1/validate", {
          method: "POST",
          headers: { "content-type": "application/json" },
          body: JSON.stringify({
            sessionId,
  ...
  const shareScore = useCallback(async () => {
    if (!results) return;
    const shareResults = results.scoreSnapshot ?? results;
    const text = `I just scored ${shareResults.elo.toLocaleString()} and cleared ${shareResults.solved}/${TOTAL_SOLVE_TARGET} on this CheetCode run (rank #${shareResults.rank}). Think your agent can beat it? 🔥`;
    const fullText = `${text}\n\n${ORIGINAL_TWEET_URL}`;
        ...
        if (validationRes.ok) {
          ...
          const outcomes = await Promise.all(
            problems.map(async (problem) => {
              ...
              try {
                const single = await clientFetch("/api/level-1/validate", {
                  method: "POST",
                  headers: { "content-type": "application/json" },
