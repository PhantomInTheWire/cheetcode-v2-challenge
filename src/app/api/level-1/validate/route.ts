...
import { buildArgs, normalizeTestCasesWithArgs } from "@/lib/game/testcaseArgs";
import {
  SANDBOX_FLAG,
...
export async function POST(request: Request) {
  return withOwnedSessionRoute<Payload>(
    request,
    {
      ...
      errorLabel: "/api/level-1/validate error",
    ...
    async ({ github, convex, session, body }) => {
      ...
      if (!result.passed) {
        result.system_note =
          ...
          "on all subsequent requests. Include this header in your next API call to /api/level-1/finish " +
